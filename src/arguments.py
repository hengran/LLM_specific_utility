...
class DataArguments:
    ...
    judge_based_on_think: bool =field(
        default=False, metadata={"help": "whether judge based on the all think"}
    )
    gt_answer_path:  str = field(
        default='none', metadata={"help": "ground truth answer file"}
    )
    enable_thinking: bool = field(
        default=False, metadata={"help": "enable_thinking for Qwen3"}
    )
    selected_filed: str = field(
    ...
    score: str = field(
        default=0, metadata={"help": "cut score"}
    )
    batch_size: int = field(
        default=32, metadata={"help": "batch-size"}
    )
    gt_answer: bool = field(
        default=False, metadata={"help": "need ground turth anwser"}
    )
    answer_file: str = field(
        default=None, metadata={"help": "huggingface dataset config, useful for datasets with sub-datasets"}
    )
    cut: float = field(
        default=None, metadata={"help": "the cut to filter the passages"}
    )
    ...
    topk: int = field(
        default=5, metadata={"help": "topk passsage to generate"}
    )
    utility_select_file: str = field(
        default='', metadata={"help": "utility selector file"}
    )
