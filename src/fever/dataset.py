...
def get_direct_judge_point_utility_no_cot_answer(question, passage, answer):
    return [
            {'role': 'user', 'content': f"Claim: {question} \n Passage: {passage} \n The reference verification: {answer} Determine if the passage has utility based on two strict criteria: \n 1. Usefulness: The passage should not only be relevant to the question but also be useful in verifying the claim.\n 2. Novelty: Is the useful information new to you? This means it must NOT be part of your pre-existing knowledge.\n"+"Directly output your response. The format of the output is: 'Utility judgment: Yes/No.'"},
           ]
...
def get_prefix_direct_judge_list_utility(query, num):
    return [
            {'role': 'user',
             'content': f"I will provide you with {num} passages, each indicated by number identifier []. \nSelect the passages that have utility for youself in verifying the claim: {query}."},
            {'role': 'assistant', 'content': 'Okay, please provide the passages.'}]
...
def get_prefix_direct_judge_list_utility_answer(query, num):
    return [
            {'role': 'user',
             'content': f"I will provide you with {num} passages, each indicated by number identifier []. I will also give you the reference verification for the claim. \nSelect the passages that have utility for youself in verifying the claim: {query}."},
            {'role': 'assistant', 'content': 'Okay, please provide the passages and the reference verification.'}]
def get_post_direct_judge_list_utility_with_answer(query, instruct, answer):
    return f"Claim: {query}.\n\n Reference verification: {answer}\n\n Determine if the passage has utility based on two strict criteria: \n 1. Usefulness: The passage should not only be relevant to the question but also be useful in verifying the claim.\n 2. Novelty: Is the useful information new to you? This means it must NOT be part of your pre-existing knowledge.\n"+instruct
#####################################################################
...
def direct_generate_answer_prompt(question):
    return [
            {'role': 'user', 'content': f"Verify the following claim based on your internal knowledge with one or few words without the explanation. Claim: {question}\n\n The final repsonse is REFUTES, SUPPORTS. Verification:"},]
...
def generate_answer_prompt_passages(question, passages):
    if isinstance(passages, list):
        ...
        pas = passages
    return [ 
            {'role': 'user', 'content': f"Information: \n{pas}\nVerify the following claim based on the given information or your internal knowledge  with one or few words without the source.\n Claim: {question}\n\n The final repsonse is REFUTES, SUPPORTS. Verification:"},]

def generate_answer_prompt_passages_without_passage(question):
    return [ 
            {'role': 'user', 'content': f"Verify the following claim based on your internal knowledge with one or few words without the source.\n Claim: {question}\n\n The final repsonse is REFUTES, SUPPORTS. Verification:"},]
...
class ListUtilityEncodeDataset(Dataset):
    def __init__(self, data_args: DataArguments, tokenizer):
        ...
        Directly output the passages you selected that have utility for yourself in verifing the claim. The format of the output is: 'My selection:[[i],[j],...].'. Only response the selection results, do not say any word or explain. 
        """
        self.answer_data = {}
        if self.data_args.gt_answer: 
            with open(self.data_args.gt_answer_path, "r", encoding="utf-8") as file_r:
...
class ListRankEncodeDataset(Dataset):
    def __init__(self, data_args: DataArguments, tokenizer):
        ...
        self.utility_instruct = """
        Directly output the ranked the passages in descending order of utility for yourself in verifing the claim. The format of the output is: '[i]>[j]>...'. Only response the ranked results, do not say any word or explain. 
        """
    def __len__(self):
        return len(self.train_data)
