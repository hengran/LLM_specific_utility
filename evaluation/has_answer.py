...
if __name__ == "__main__":
    ...
    with open("utility_alignment/datasets/hotpotqa/postive.jsonl", "r", encoding="utf-8") as file_r:
        ...
        for data_type in ["nq", "hotpotqa", "2wikiqa", "msmarco", "triviaqa"]:
            query_id_answers = {}
            ...
            for model_name in ["llama31-8b-instruct", "Qwen3-8B", "Qwen3-14B", "Qwen3-14B"]:
                root = "utility_alignment/results/main_experiments/pseudo_answer/"+data_type+"/"
                file_list = [
                    "_point_verberlized",
                    "_list_verberlized",
                    ]
                for file_name in file_list:
                    ...
                    outdir = open(file_+"_result.jsonl", 'w', encoding='utf-8')
                    query_ids = []
                    for line in infile.readlines():
                        ...
                        if d["query_id"] not in query_id_answers:
                            continue
                        d['EM'], d['F1'] = deal_answer(answer_, query_id_answers[d["query_id"]])
                        d['gt_has_weight'] = has_answer(query_id_answers[d["query_id"]], answer_, match_type="string")
                        d["gt_answer"] = query_id_answers[d["query_id"]]
                        formated_passages_ids = d["formated_passages_ids"]
                        outdir.write(json.dumps(d) + '\n')
                        em_scores.append(d['EM'])
                        f1_scores.append(d['F1'])
                        has_scores.append(d['gt_has_weight'])
                    infile.close()
                    outdir.close()
                    em_scores_mean = np.mean(em_scores)
                    f1_scores_mean = np.mean(f1_scores)
                    has_scores_mean = np.mean(has_scores)
                    print(f"{data_type}\t {model_name}\t {file_name}\t {em_scores_mean*100} \t {f1_scores_mean*100} \t {has_scores_mean*100} ")
