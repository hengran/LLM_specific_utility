        ...
        if model_name == "llama31-8b-instruct":
            with open("utility_alignment/results/pseudo_answer/"+data_type+"/"+model_name+"_no_passages_results.jsonl", "r", encoding="utf-8") as file_r:
                for line in file_r:
                    ...
                    query_id_scores[query_id] = js["gt_has_weight"]
        else:
            with open("utility_alignment/results/pseudo_answer/"+data_type+"/"+model_name+"_no_passages_results.jsonl", "r", encoding="utf-8") as file_r:
                for line in file_r:
                    js = json.loads(line)
                ...
                for line in file_r:
                    ...
                    if "hotpotqa" in data_type:
                        if query_id not in hotpotqa_filter:
                            continue
                    utility_passages =  [passage for passage in js["gt_has_weight"] if passage["utility_score"]-query_id_scores[query_id] > 0]
                    passage_score = [passage["utility_score"]-query_id_scores[query_id] for passage in js["gt_has_weight"]]
                    utility = [score for score in passage_score if score >0]
                    original = [passage["utility_score"]==1 for passage in js["gt_has_weight"]]
