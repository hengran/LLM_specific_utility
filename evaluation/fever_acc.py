...
if __name__ == "__main__":
    for data_type in ["fever"]:
        ...
        root = ""
        file_list = [
            "llama31-8b-instruct_no_passages",
            "llama31-8b-instruct-all-passages",
            "Qwen3-14B_no_passages",
            "Qwen3-14B-all-passages",
            "Qwen3-32B_no_passages",
            "Qwen3-32B-all-passages",
            "Qwen3-8B_no_passages"
            ]
        for file_name in file_list:
            ...
            for line in infile.readlines():
                ...
                if d["query_id"] not in query_id_answers:
                    continue
                d['acc'] = has_answer(query_id_answers[d["query_id"]], answer_, match_type="string")
                d["gt_answer"] = query_id_answers[d["query_id"]]
                outdir.write(json.dumps(d) + '\n')
                has_scores.append(d['acc'])

            infile.close()
            outdir.close()
            has_scores_mean = np.mean(has_scores)
            print(f"{len(has_scores)} \t {data_type}/{file_name}:  \t {has_scores_mean*100} ")
